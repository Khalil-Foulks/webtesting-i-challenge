...
describe("enhancer", () => {
    ...
    describe("fail", () => {
        it("should decrease durability by 5 if enhancement < 15, decrease durability by 10 if enhancement > 15 and if enhancement > 16 decrease enhancement by 1", () => {
            ...
            expect(enhancer.fail({ name:"spear", durability: 5, enhancement: 16 })).toEqual({ name:"spear", durability: 0, enhancement: 16 })
            expect(enhancer.fail({ name:"staff", durability: 1, enhancement: 0 })).toEqual({ name:"staff", durability: 0, enhancement: 0 })

            expect(enhancer.fail({ name:"2h sword", durability: 50, enhancement: 19 })).not.toEqual({ name:"2h sword", durability: 90, enhancement: 18 })
            expect(enhancer.fail({ name:"armor", durability: 50, enhancement: 19 })).not.toEqual({ name:"armor", durability: 90, enhancement: 20 })
